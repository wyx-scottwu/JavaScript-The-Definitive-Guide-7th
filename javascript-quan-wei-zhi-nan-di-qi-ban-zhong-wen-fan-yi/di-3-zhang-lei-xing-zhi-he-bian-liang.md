...
It is a common (but not universal) convention to declare constants using names with all capital letters such as H0 or HTTP\_NOT\_FOUND as a way to distinguish them from variables.

> 通常(但不是通用的)约定是使用全大写字母(如H0或HTTP\_NOT\_FOUND)的名称来声明常量，以将它们与变量区分开来。
